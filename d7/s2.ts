...
function countCharsWithWildCard(str: string): Record<string, number> {
  const mapping = str.split("").reduce(
    (map, char) => {
      if (map[char]) {
        map[char]++;
      } else {
  ...
  const { J, ...otherCards } = mapping;

  if (mapping["J"]) {
    var maxChar: string | undefined = undefined;
    Object.entries(mapping).forEach(([char, value]) => {
      if (char !== "J" && (!maxChar || value > mapping[maxChar])) {
        maxChar = char;
      }
    });
    if (maxChar) {
      mapping[maxChar] += mapping["J"];
      //   Joker have been consumed
      mapping["J"] = 0;
    }
  }
...
async function run(filename: string) {
  ...
  const handsType = hands.reduce(
    (rankMap, hand, index) => {
      const charsOccur = countCharsWithWildCard(hand);
  ...
  for (let groupId = 1; groupId <= 7; groupId++) {
    ...
    console.log("Group", groupId, "=>", sortedGroup);
    ...
    hands
      .map((hand, index) => {
        return {
          hand,
          rank: handsRank[hand],
          value: handsRank[hand] * bids[index],
          bid: bids[index],
        };
      })
